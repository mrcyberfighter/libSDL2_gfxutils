 ...
 * Compilation:
 *
 * $ cc -Wall -Wextra balls_trajectory.c -lm -lSDL2 -lSDL2_gfx -lSDL2_gfxutils
 *
 * *********************************************************************/
...
#define BORDERS_WIDTH (32+16)

#define WIDTH_BORDERS  (800-BORDERS_WIDTH)
#define HEIGHT_BORDERS (600-BORDERS_WIDTH)

float SPEED = 0.225f / 2.0f;
...
typedef struct Ball_ {
  ...
  uint32_t index;     /** The current index from the array positions.                         **/
  uint32_t steps;     /** The number of Pixel contains in the array position.                 **/
  int8_t x_dir;       /** Indication of the x direction: -1 = to the left ; 1 = to the right. **/
  int8_t y_dir;       /** Indication of the y direction: -1 = to up       ; 1 = to down.      **/
  Pixel *positions;   /** Arrays from Pixel for trajectory computing.                         **/
...
inline int get_rand(int modulo) {
  ...
  int random;
  return (random = (rand() % modulo)) <= BORDERS_WIDTH * 2 ? BORDERS_WIDTH : random;
}

Ball *init_ball(char color) {

  /** Allocated and initialized a Ball structure. **/
  ...
  x = get_rand(WIDTH_BORDERS);
  y = get_rand(HEIGHT_BORDERS);

  Line trajectory;
  /** Start Pixel of trajectory. **/
  trajectory.xy1.x = x;
  trajectory.xy1.y = y;

  if (rand() % 2) {
    x = (rand() % 2) ? BORDERS_WIDTH : WIDTH_BORDERS;
    y = get_rand(HEIGHT_BORDERS);
  }
  else {
    x = get_rand(WIDTH_BORDERS);
    y = (rand() % 2) ? BORDERS_WIDTH : HEIGHT_BORDERS;
  }

  /** End Pixel of trajectory. **/
  trajectory.xy2.x = x;
  trajectory.xy2.y = y;
  ...
  float length = get_distance_pixels(trajectory.xy1, trajectory.xy2);

  ball->steps = (uint32_t) roundf(length * SPEED);

  ball->positions = (Pixel *) calloc((ssize_t) ball->steps, sizeof(Pixel));

  compute_trajectory(ball->positions, &trajectory, ball->steps);
...
void translate_ball(Ball * ball) {
  ...
  for (c = 0; c < 25; c++) {
    set_form_center(ball->ball[c], ball->positions[ball->index], true);
  }
...
void check_collide_borders(Ball * ball) {
  ...
  if (ball->steps - 1 < ball->index) {
    /** Ball is not at the end of the trajectory (not at a border). **/
    return;
  }

  if (ball->steps - 1 == ball->index) {

    /** Ball is at end of trajectory so colliding a border. **/

    Pixel cur_pos;
    cur_pos = ball->positions[ball->index];  /** Getting current point from trajectory.  **/

    free(ball->positions);

    Line trajectory;
    trajectory.xy1 = cur_pos;  /** New trajectory start point.  **/


    if (ball->x_dir == -1 && ball->y_dir == -1) {

      /** Ball previous direction was to the left and up.  **/

      /** Invert x direction.  **/
      trajectory.xy2.x = WIDTH - BORDERS_WIDTH;
      ball->x_dir = 1;

      /** Random y end coordinates.  **/
      trajectory.xy2.y = get_rand(HEIGHT_BORDERS);
      if (trajectory.xy2.y <= HEIGHT / 2) {
        ball->y_dir = -1;
      }
      else {
        ball->y_dir = 1;
    ...
    else if (ball->x_dir == 1 && ball->y_dir == -1) {

      /** Ball previous direction was to the right and up.  **/

      /** Invert y direction.  **/
      trajectory.xy2.y = HEIGHT - BORDERS_WIDTH;
      ball->y_dir = 1;

      /** Random x end coordinates.  **/
      trajectory.xy2.x = get_rand(WIDTH_BORDERS);
      if (trajectory.xy2.x <= WIDTH / 2) {
        ball->x_dir = -1;
      }
      else {
        ball->x_dir = 1;
      ...
      /** Ball previous direction was to the right and down. **/

      /** Invert x direction.  **/
      trajectory.xy2.x = BORDERS_WIDTH;
      ball->x_dir = -1;
      ...
      /** Ball previous direction was to the left and down.  **/
